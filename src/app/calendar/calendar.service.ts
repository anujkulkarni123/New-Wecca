...
export class CalendarService {
  ...
  addRecurringDates(dateObject: any) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      switch (additions) {
        ...
        case "biweekly":
          start = addWeeks(
            new Date(dateObject["start"]["dateTime"]),
            2 * (i + 1)
          );
  ...
  getData(calendar: string) {
    //Exec doesn't work rn
    const CALENDAR_ID =
      calendar === "exec" ? KEYS.EXEC_CALENDAR_ID : KEYS.GENERAL_CALENDAR_ID;
    const KEY = KEYS.KEY;
